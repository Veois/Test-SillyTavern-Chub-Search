...
const defaultSettings = {
    findCount: 30, // Updated to display 30 cards per page
    nsfw: false,
};
...
async function downloadCharacter(input) {
    ...
    let request = await fetch('/api/content/importUUID', {
        method: 'POST',
...
async function fetchCharactersBySearch({ searchTerm, includeTags, excludeTags, nsfw, sort, page=1 }) {
    ...
    let first = extension_settings.chub.findCount; // Ensures it uses the updated findCount value
    let asc = false;
    ...
    characterBlobs.forEach((character, i) => {
        ...
        chubCharacters.push({
            ...
            tags: searchData.nodes[i].topics.slice(0, 5), // Limit tags to 5
...
${chubCharacters.map((character, index) =>
generateCharacterListItem(character, index)
).join('')}
...
<input type="text" id="characterSearchInput" class="text_pole flex1"
placeholder="Search CHUB for characters...">
...
<input type="text" id="includeTags" class="text_pole flex1"
...
<input type="text" id="excludeTags" class="text_pole flex1"
...
<button class="menu_button" id="pageDownButton"><i
class="fas fa-chevron-left"></i></button>

<label for="pageNumber">Page:</label>

<input type="number" id="pageNumber"
class="text_pole textarea_compact wide10pMinFit"
min="1"
value="1">

<button class="menu_button" id="pageUpButton"><i
...
${Object.keys(readableOptions).map(key =>
`<option value="${key}">${readableOptions[key]}</option>`
).join('')}
...
characterListContainer.addEventListener(
'click',
function (event)
{
...
const executeCharacterSearchDebounced =
debounce((options) =>
executeCharacterSearch(options), 750);
...
const includeTags =
splitAndTrim(document.getElementById(
'includeTags').value);
...
if (
e.target.id !== 'pageNumber'
&& e.target.id !== 'pageUpButton'
&& e.target.id !== 'pageDownButton') {

// Page being updated to start from page 1

// If the page number is not being changed by up/down button update

// or direct manual input
...
// If page is below min limit reset to page start value
...
pageNumber.value =
clamp(parseInt(pageNumber.value)
+ 1, 0,
...
console.log(`Request failed for ${fullPath},
...
{
$("#external_import_button").after('<button id="search-chub"
class="menu_button fa-solid fa-cloud-bolt faSmallFontSquareFix"
title="Search CHub for characters"></button>');
